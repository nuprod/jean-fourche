...
class AccountVoucherWizard(models.TransientModel):
    ...
    payment_ref = fields.Char("Ref.",required=True)
    ...
    # def make_advance_payment(self,sale_ids=None):
    ...
    #         _logger.error(self.journal_id)
    #         acquirer_id = len(self.journal_id.acquirer_ids)>0 and self.journal_id.acquirer_ids[0]
                        
    #         if not acquirer_id:
    #             raise UserError("Aucun intermédiaire de paiement sur ce journal et cette méthode de paiements, impossible de continuer")
    ...
    #         transaction = self.env['payment.transaction'].sudo().with_context(lang=None).create(values)
            
    #         transaction._set_transaction_done()
    #         transaction._reconcile_after_transaction_done()
            
    #         transaction.payment_id.sale_id = sale.id
            
    #         sale.transaction_ids = [(6,0,[transaction.id])]
    #         _logger.info(sale.transaction_ids)
    ...
    def make_advance_payment(self):
        ...
        if sale_ids:
            sale_id = fields.first(sale_ids)
            sale = sale_obj.browse(sale_id)
            _logger.error(f"sale sale sale sale sale {sale}")
            payment_vals = self._prepare_payment_vals(sale)
            _logger.error(f"payment :  {payment_vals}")
